...
{bin,libs,sources code} that make up your application or os. **container**: A container is an instance that image running as a process.
...
- get port 80 in the host ip and link with the container ip port 80: `{host ip -> 80 : 80 <- container ip}`
...
this will show the process running inside the container webhost.

`docker container --help **~ docker --help**`

list the commands that can be applied in this container.

_> "From now on, I will not write the old way or new way of writting the docker commands unless it's necessary"_
